# -*- coding: utf-8 -*-
"""
parameter_learning_patch_geometry.py
=========================================
Parameter learning framework for 3D patient specific LA. The code is
adapted to use the already-extracted patch_geometry.npz (produced once, locally) 
instead of the full clinical mesh withheld and not shared on repository 
due restrictions on sharing/distribution of patient clinical data. Omnipolar EGMs 
are generated insilico and no patient data has been uploaded or shared as
part of ground truth omnipolar EGMs. 

Inputs:
  - patch_geometry.npz (from part1_extract_patch_geometry.py)
  - gt2_egm_{case}.npz for each case (from
    part2_patch_differentiability_showcase.py, updated to save the
    complete set of GT features -- p2ps_s2_gt/p2ps_s1_gt/ari_s1_gt/
    ari_s2_gt/at_s2_gt/slew_s2_gt/p2p_s2_gt -- matching the original
    GT generator own output exactly, not a partial subset)
 
Outputs:
  - results_{case}.npz for each case (recovered parameters, Phase 1/2
    losses, evaluation counts, wall time)
 
Usage: to estimate parameters for a different case (or subset of
cases), edit the CASES list below (default: ['set1', 'set2', 'set3']).
...
PATCH_GEOMETRY_PATH = "/path_to_dir/patch_geometry.npz"
GT_EGM_DIR = "/path_to_dir/egm_pipeline_for_jax_set1"
OUT_DIR = "/path_to_dir/working" if os.path.isdir("/path_to_dir/working") else "."
...
# ══════════════════════════════════════════════════════════════════════════
# [1] LOAD EXTRACTED PATCH GEOMETRY 
# ══════════════════════════════════════════════════════════════════════════
print(f"Loading extracted patch geometry: {PATCH_GEOMETRY_PATH}")
_pg = np.load(PATCH_GEOMETRY_PATH, allow_pickle=True)
lv_full = np.array(_pg["lv"], dtype=np.float64)
le_full = np.array(_pg["le"], dtype=np.int64)
HD16_full = np.array(_pg["HD16"], dtype=np.float64)
fib3_full = np.array(_pg["fib3"], dtype=np.float64)
cs_edge_full = str(_pg["cs_edge"][0])
print(f"  {len(lv_full):,} nodes  {len(le_full):,} elements  "
      f"pacing edge [{cs_edge_full}]")
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# [3] PATCH BUILDER -- geometry (lv, le, HD16, fib3, cs_edge)
#     comes directly from the already-extracted patch_geometry.npz
# ══════════════════════════════════════════════════════════════════════════
def build_patch(case):
    lv, le = lv_full, le_full
    HD16, fib3, cs_edge = HD16_full, fib3_full, cs_edge_full
    Np = len(lv)
...
for CASE in CASES:
    ...
    print(f"CASE: {CASE}")
    print(f"{'='*60}")
    gt_data  = np.load(os.path.join(GT_EGM_DIR, f"gt2_egm_{CASE}.npz"),
                       allow_pickle=True)
    ...
    print(f"  Loaded gt2_egm_{CASE}.npz")
    ...
    # Save results
    out_path=os.path.join(OUT_DIR, f"results_{CASE}.npz")
    np.savez(out_path,
...
for case in CASES:
    fpath = os.path.join(OUT_DIR, f"results_{case}.npz")
