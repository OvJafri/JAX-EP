...
CARP_MESH_DIR = (
    ...
    r"\Jax_codes\Jax_EP\Results\benchmark"
)

CARP_IGB_PATH = (
    r"C:\Users\exx915\Documents\DERIstuff\Brompton_Projects"
    r"\Jax_codes\Jax_EP\Results\benchmark\JAX_EP_benchmark.igb"
)

CARP_MESH_NAME = "HD_Grid_20x7_200um"

# Laptop output folder
OUT_DIR = (
    r"C:\Users\exx915\Documents\DERIstuff\Brompton_Projects"
    r"\Jax_codes\Jax_EP\Results\benchmark\github_repo\2D_validation"
)
...
def build_mesh(dx_mm):
    ...
    x_mm = (
        carp_pts[:, 0] - carp_pts[:, 0].min()
    ) * 1e-3

    y_mm = (
        carp_pts[:, 1] - carp_pts[:, 1].min()
    ) * 1e-3
    ...
    x_unique = np.unique(np.round(x_mm, 4))
    y_unique = np.unique(np.round(y_mm, 4))

    nx = len(x_unique)
    ny = len(y_unique)

    node_grid = np.arange(n_carp).reshape(ny, nx)
...
def build_fem(V_mm, E):
    ...
    return (
        m_j,
        jnp.array(eu, dtype=jnp.int32),
        jnp.array(ev, dtype=jnp.int32),
        jnp.array(w_aniso, dtype=jnp.float64)
    )
...
valid_mask = (
    np.isfinite(jax_at_valid)
    &
    np.isfinite(carp_at_valid)
)
...
ax_c.set_title(
    ...
    f"L2={l2_err:.3f}ms  Max={max_err:.3f}ms",
    fontweight="bold",
    pad=4
)
...
ax_e.axvline(
    ...
    lw=1.5,
    ls="--",
    label=f"Mean bias = {mean_err:.3f} ms"
)
...
summary = (
    ...
    f"CV diff:   {abs(jax_cv - carp_cv) * 100:.2f}%\n"
    f"Nodes:     {valid_mask.sum():,} (direct)"
)
